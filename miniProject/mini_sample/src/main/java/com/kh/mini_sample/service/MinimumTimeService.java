...
public class MinimumTimeService {
    ...
    //노드의 크기, 출발지
    public PathVO Dijkstra(String start, String dest, StationVO[] stations, ArrayList<NodeDistVO>[] orinGraph,List<Boolean>whatDayList) throws ParseException {
        ...
        List<Date> shortest = new ArrayList<>();
        ...
        for(int s = 0; s<stationMap.get(start).size(); s++ ){
            ...
            for(int d = 0; d<stationMap.get(dest).size(); d++) {
                ArrayList<NodeDistVO>[] graph = deepCopy(orinGraph);
                ...
                while (!pq.isEmpty()) {
                    ...
                    for (NodeDistVO next : graph[nowVertex]) {
                        boolean whatDay;
                                ...
                                for (int j = 0; j < whatDayDpt.size(); j++) {
                                    ...
                                    if(nowVertex != startIndex && !trainNumber[nowVertex].equals(whatDayDptTrn.get(j))){
                                        if((!trainNumber[nowVertex].equals("trans")) &&stations[nowVertex].getLn_cd().equals(stations[next.getIndex()].getLn_cd())){
                                            waitTime = (dptTable.getTime() - (arvTm[nowVertex].getTime()+WALK_TIME)); //열차가 오는데 걸리는시간
                                            if(waitTime >= 0) waitTime+=WALK_TIME;
                                        }
                                    }
                        ...
                        if (next.getCost() != INF){
                            if(dist[next.getIndex()] > dist[nowVertex] + next.getCost()) {
                                ...
                                pq.offer(new NodeDistVO(next.getIndex(), dist[next.getIndex()], totalTrans[next.getIndex()]));
                ...
                if(shortest.size() > 1){
                    if(shortest.get(0).getTime() < shortest.get(1).getTime()) {
                        ...
                        if(Index0 >= Index1) removeList(arvTmList,dptTmList,distList,prevList,shortest,startList,destList,0);
                        else removeList(arvTmList,dptTmList,distList,prevList,shortest,startList,destList,1);
                    }
    ...
    public ArrayList<NodeDistVO>[] deepCopy(ArrayList<NodeDistVO>[] graph) {
        ArrayList<NodeDistVO>[] newGraph = new ArrayList[graph.length];
        for (int i = 0; i < graph.length; i++) {
            newGraph[i] = new ArrayList<NodeDistVO>();
            for (int j = 0; j < graph[i].size(); j++) {
                NodeDistVO node = new NodeDistVO(graph[i].get(j).getIndex(),graph[i].get(j).getCost(),graph[i].get(j).getCount());
                newGraph[i].add(node);
            }
        }
        return newGraph;
