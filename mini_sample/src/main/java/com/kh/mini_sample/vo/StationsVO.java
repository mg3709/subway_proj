...
public class StationsVO {
    ...
    StationVO[] stations;
    ArrayList<NodeDistVO>[] graphDist;
    ArrayList<NodeTransVO>[] graphTrans;
    @Autowired
    public StationsVO(Common common)  {
        ...
        try (Connection connection = jdbcTemplate.getDataSource().getConnection();
            ...
            while (stationResultSet.next()) {
                ...
                stations[index] = new StationVO(index, ln_cd, name, edge, trans, transTime);
            }
            ...
            while (timetableResultSet.next()) {
                ...
                for (int i = 0; i < jsonArray.length(); i++) {
                    ...
                    updateScheduleArray(index, stations, arvTime, dptTime, arvTrnNo, dptTrnNo, dayCd);
                }
            }
        ...
        //////////////////////////////////////////////////그래프 초기화문구
        graphDist = new ArrayList[STATION_SIZE];
        graphTrans = new ArrayList[STATION_SIZE];
        for (int i = 0; i < STATION_SIZE; i++)  {
            graphTrans[i] = new ArrayList<>();
            graphDist[i] = new ArrayList<>();
        }
    ...
    public void addEdgesToGraph(ArrayList<NodeDistVO>[] graphDist, ArrayList<NodeTransVO>[] graphTrans, StationVO[] stations) {
        for (int i = 0; i < stations.length; i++) {
            for (int j = 0; j < stations[i].getEdge().size(); j++) {
                ...
                Integer next = Integer.parseInt(edgeStr);
                graphDist[i].add(new NodeDistVO(next, INF, 0));
                graphDist[next].add(new NodeDistVO(i, INF, 0));
                graphTrans[i].add(new NodeTransVO(next, INF, 0));
                graphTrans[next].add(new NodeTransVO(i, INF, 0));
            }
        }
        ...
        for (int i = 0; i < stations.length; i++) {
            for (int j = 0; j < stations[i].getTransfer().size(); j++) {
                ...
                Integer next = Integer.parseInt(transferStr);
                Integer time = Integer.parseInt(stations[i].getTransTime().get(j));
                time = time * 820;
                graphDist[i].add(new NodeDistVO(next, time, 0));
                graphDist[next].add(new NodeDistVO(i, time, 0));
                graphTrans[i].add(new NodeTransVO(next, time, 0));
                graphTrans[next].add(new NodeTransVO(i, time, 0));
            }
        }
    }
    public void updateScheduleArray(int index, StationVO[] stations, String arvTime, String dptTime, String arvTrnNo, String dptTrnNo, String dayCd) {
        if (dayCd.equals("8")) {
            updateScheduleHour(stations[index].getWeekArvTime(), arvTime, stations[index].getWeekArvTrnNo(),arvTrnNo);
            updateScheduleHour(stations[index].getWeekDptTime(), dptTime, stations[index].getWeekDptTrnNo(),dptTrnNo);
        } else if (dayCd.equals("9")) {
            updateScheduleHour(stations[index].getHoliArvTime(), arvTime, stations[index].getHoliArvTrnNo(),arvTrnNo);
            updateScheduleHour(stations[index].getHoliDptTime(), dptTime, stations[index].getHoliDptTrnNo(),dptTrnNo);
        }
    }

    public void updateScheduleHour(List<String>[] scheduleHour, String time, List<String>[] scheduleTrn,String trnNo) {
        if (time.length() >= 6) {
            int hour = Integer.parseInt(time.substring(0, 2));
            List<String> hourList = scheduleHour[hour];
            List<String> trnList = scheduleTrn[hour];
